...
warn "driver.pl: config.pm not found; continuing without scoreboard metadata.\n"
    unless $HAS_CONFIG;
...
#pss
if ($HAS_CONFIG) {
    $config::total_c_points = $total_c_points;
    $config::total_c_rating = $total_c_rating;
    $config::total_p_points = $total_p_points;
    $config::total_p_rating = $total_p_rating;
}
